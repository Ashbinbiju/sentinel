...
async function debugWelcorp() {
  const candles = await fetchCandles("WELCORP");
  const d25 = candles.filter(c => getISTDateStr(c.t) === "2026-06-25");
  
  for (let c of d25) {
      const timeStr = new Date((c.t + 300) * 1000 + 19800 * 1000).toISOString().substr(11, 5);
      if (timeStr >= "09:45" && timeStr <= "10:30") {
          console.log(timeStr, c);
      }
  }
}
debugWelcorp().catch(console.error);
