...
async function runBacktest() {
    ...
    console.log(`Fetching watchlist snapshots for ${testDate} from database...`);
    
    let snapshots: any[] = [];
    try {
        snapshots = await db.select().from(watchlistSnapshotsTable).where(eq(watchlistSnapshotsTable.date, testDate));
    } catch (e: any) {
        console.error("Failed to fetch snapshots:", e.message);
    }
    
    if (snapshots.length === 0) {
        console.error("No watchlist snapshots found in database for testDate.");
        return;
    }
    
    const uniqueSymbolsMap = new Map<string, string>();
    for (const snap of snapshots) {
        if (!uniqueSymbolsMap.has(snap.symbol)) {
            uniqueSymbolsMap.set(snap.symbol, snap.category);
        }
    }
    
    testCases = Array.from(uniqueSymbolsMap.entries()).map(([sym, cat]) => ({
        symbol: sym,
        date: testDate,
        category: cat
    }));
    ...
    for (const { symbol: sym, date, category } of testCases) {
        if (sym === "STLTECH" || sym === "MTARTECH" || sym === "CAMS" || sym === "ACC") {
            console.log(`Skipping ${sym} because Upstox lacks its mapping and AngelOne hangs.`);
            continue;
        }
